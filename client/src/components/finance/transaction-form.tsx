...
const TransactionForm = ({ isOpen, onClose, transactionId }: TransactionFormProps) => {
  ...
  const formSchema = insertTransactionSchema.extend({
    type: z.enum([TransactionType.INCOME, TransactionType.EXPENSE], {
      required_error: "Por favor selecione o tipo da transação",
    }),
    status: z.enum([
  ...
  const isLoading = isLoadingTransaction || createMutation.isPending || updateMutation.isPending;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit 
              ? "Atualize os dados da transação conforme necessário."
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      disabled={isLoading}
                      placeholder="Ex: NF-001, Recibo-123, etc."
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
